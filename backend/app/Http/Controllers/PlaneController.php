...
class PlaneController extends Controller
{
    public function index(Request $request){
        $collection = $this->getPlanesCollection();

        if($request->get('manufacturer') != ""){
            $collection = $collection->filter(function($plane) use ($request){
                return $plane['manufacturer'] == $request->get('manufacturer');
            });
        }
        if($request->get('type') != ""){
            $collection = $collection->filter(function($plane) use ($request){
                return $plane['type'] == $request->get('type');
            });
        }
        if($request->get('minSeats') != ""){
            $collection = $collection->filter(function($plane) use ($request){
                return $plane['seats'] >= $request->get('minSeats');
            });
        }
        if($request->get('maxSeats') != ""){
            $collection = $collection->filter(function($plane) use ($request){
                return $plane['seats'] <= $request->get('maxSeats');
            });
        }

        if($request->get('orderBy') != ""){
            if($request->get('order')=='desc'){
                $collection = $collection->sortByDesc($request->get('orderBy'));
            }else{
                $collection = $collection->sortBy($request->get('orderBy'));
            }
        }

        return view('plane.index',[
            'title'=>'Repülők ('.$collection->count().' db)',
            'planes'=>$collection,
            'query'=>$request->all(),
            'maxSeats' => $this->getPlanesCollection()->sortByDesc('seats')->first(),
            'oldest' => $this->getPlanesCollection()->sortBy('first_flight')->first(),
            'averageSeats'=>$this->getPlanesCollection()->avg('seats')
        ]);
    }
